...
for line in open("directors.txt", encoding='utf-8', errors='ignore'):
    items = line.replace("\n", "").split("\t")
...
fig, ax = plt.subplots()
b = ax.barh(range(len(actors)), fans, color='#FF8C00')

# # 设置Y轴纵坐标上的刻度线标签。
ax.set_yticks(range(len(actors)))
ax.set_yticklabels(actors, color='#9400D3')
...
plt.title('最受欢迎导演Top30', loc='center', fontsize='14',
